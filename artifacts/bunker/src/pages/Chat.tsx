...
import { useGetMessages, useSendMessage, useBurnHistory } from "@workspace/api-client-react";
...
export default function Chat() {
  ...
  const char = AI_CHARACTERS.find(c => c.id === id) ?? {
    id: id ?? "echo", name: "Unknown", avatar: "?", status: "online" as const,
    specialty: "Unknown", description: "", color: { hex: "#00f0ff" } as any,
    greeting: t("chat.greeting"),
  };
  const neon = char.color.hex;

  const { data: apiMessages, refetch } = useGetMessages(id ?? "", { query: { retry: 1, enabled: !!id } });

  const sendMutation = useSendMessage({
    mutation: { onSuccess: (msg) => setLocalMessages(prev => [...prev, msg]) },
  });
  ...
  const burnMutation = useBurnHistory({
    mutation: {
      onSuccess: () => {
        setBurning(true);
        setTimeout(() => { setLocalMessages([]); setBurning(false); refetch(); }, 600);
      },
    },
  });

  useEffect(() => {
    const greeting = { id: "greeting", characterId: id, content: char.greeting, role: "assistant", timestamp: new Date().toISOString() };
    if (apiMessages && apiMessages.length > 0) setLocalMessages(apiMessages);
    else setLocalMessages([greeting]);
  }, [apiMessages]);
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const userMsg = { id: `u_${Date.now()}`, characterId: id, content: input, role: "user", timestamp: new Date().toISOString() };
  ...
  const handleBurnClick = () => {
    if (!burnConfirm) { setBurnConfirm(true); setTimeout(() => setBurnConfirm(false), 3000); return; }
  ...
  return (
    <div className="flex flex-col h-screen relative overflow-hidden" style={{ background: "#050508" }}>
      ...
      <header className="flex items-center justify-between px-4 py-3 z-10 sticky top-0"
        style={{ background: "rgba(5,5,10,0.85)", borderBottom: `1px solid ${neon}25`, backdropFilter: "blur(16px)", boxShadow: `0 4px 20px ${neon}08` }}>
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-display font-bold text-base tracking-widest uppercase text-white" style={{ textShadow: T.glowText(neon) }}>{char.name}</h2>
          </div>
          <div className="flex items-center gap-1.5">
            <ShieldCheck className="w-3 h-3" style={{ color: "#00ff88" }} />
            <span className="font-tech text-[9px] tracking-widest uppercase" style={{ color: "#00ff88" }}>{t("chat.encrypted")}</span>
          </div>
        </div>
        <motion.button onClick={handleBurnClick} disabled={burnMutation.isPending} whileTap={{ scale: 0.9 }}
          className="p-2 -mr-2 flex flex-col items-center gap-0.5 transition-all"
          style={{ color: burnConfirm ? "#ff3366" : "#444", filter: burnConfirm ? `drop-shadow(${T.glow("#ff3366")})` : "none" }}>
          <Flame className={`w-6 h-6 ${burnConfirm ? "animate-pulse" : ""}`} />
          <span className="font-tech text-[8px] tracking-wider uppercase">{burnConfirm ? t("chat.burnConfirm") : t("chat.burn")}</span>
        </motion.button>
      </header>
      ...
      <div className="z-10 px-4 pt-4">
        <div className="flex items-center justify-center py-1.5 rounded-sm"
          ...
          <span className="font-tech text-[9px] tracking-[0.25em] uppercase" style={{ color: `${neon}80` }}>{t("chat.sessionStart")}</span>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 z-10 no-scrollbar">
        <AnimatePresence>
          {burning && (
            <motion.div key="burn-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 z-50 pointer-events-none flex items-center justify-center"
              style={{ background: "rgba(255,0,0,0.08)" }}>
              <motion.div animate={{ scale: [1, 1.4, 0.8, 1.2, 0], opacity: [1, 0.8, 1, 0.5, 0] }} transition={{ duration: 0.6 }}
        ...
        <AnimatePresence initial={false}>
          {localMessages.map((msg) => {
            ...
            return (
              <motion.div key={msg.id} initial={{ opacity: 0, y: 10, x: isUser ? 16 : -16 }}
                animate={{ opacity: 1, y: 0, x: 0 }} exit={{ opacity: 0, scale: 0.8 }}
                transition={{ type: "spring", stiffness: 300, damping: 25 }}
                className={`flex w-full ${isUser ? "justify-end" : "justify-start"}`}>
                {!isUser && (
                  <div className="flex-shrink-0 mr-2 mt-1">
                    <div className="w-7 h-7 rounded-sm flex items-center justify-center text-sm"
                      style={{ background: `${neon}15`, border: `1px solid ${neon}30` }}>{char.avatar}</div>
                  </div>
                )}
                <div className="max-w-[78%] px-4 py-3 relative"
                  style={{ background: isUser ? `${neon}10` : "rgba(20,20,30,0.8)",
                    border: `1px solid ${isUser ? `${neon}35` : "rgba(255,255,255,0.07)"}`,
                    boxShadow: isUser ? `0 0 12px ${neon}12` : undefined,
                    borderRadius: isUser ? "12px 2px 12px 12px" : "2px 12px 12px 12px" }}>
                  <div className="absolute w-2 h-2 border-t" style={{ top: 0, [isUser ? "right" : "left"]: 0,
          ...
          {sendMutation.isPending && (
            <motion.div key="typing" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex justify-start">
              <div className="px-5 py-3 flex items-center gap-1.5"
                style={{ background: "rgba(20,20,30,0.8)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "2px 12px 12px 12px" }}>
                {[0, 0.2, 0.4].map((d, i) => (
      ...
      <div className="px-4 pb-5 pt-3 z-10"
        style={{ background: "rgba(5,5,10,0.9)", borderTop: `1px solid ${neon}20`, backdropFilter: "blur(16px)" }}>
        <form onSubmit={handleSend} className="flex gap-2">
          <input type="text" value={input} onChange={e => setInput(e.target.value)}
            ...
            style={{ background: "rgba(0,0,0,0.5)", border: `1px solid ${input ? `${neon}50` : "rgba(255,255,255,0.08)"}`,
              color: neon, boxShadow: input ? `inset 0 0 10px ${neon}08` : undefined, caretColor: neon }} />
          <motion.button type="submit" disabled={!input.trim() || sendMutation.isPending} whileTap={{ scale: 0.9 }}
