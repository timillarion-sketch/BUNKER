...
export default function ChatsList() {
  return (
    <div className="min-h-screen pb-28 px-4 pt-12 no-scrollbar overflow-y-auto">
      ...
      {/* Info banner */}
      <div
        className="mb-6 p-3 flex gap-3 items-start rounded-sm"
        style={{
      ...
      >
        ...
        <div>
          <p className="font-tech text-[10px] text-white uppercase tracking-wider mb-0.5">Total Anonymity Protocol</p>
          <p className="font-sans text-[10px] text-gray-500 leading-relaxed">
            Messages are zero-knowledge encrypted and routed through decentralized nodes. Metadata stripped.
          </p>
        </div>
      </div>
        ...
        {CONVERSATIONS.map((chat, i) => (
          ...
          >
            ...
            <div className="relative shrink-0">
              <div
                className="w-12 h-12 rounded-sm flex items-center justify-center font-display font-black text-xl text-black uppercase"
                style={{
                  background: `linear-gradient(135deg, ${chat.color}, ${chat.color}80)`,
                  boxShadow: T.glow(chat.color),
                }}
              >
                {chat.name.charAt(0)}
              </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline justify-between mb-1">
                <h3
                  className="font-display font-bold text-sm text-white uppercase tracking-wide truncate transition-colors group-hover:text-current"
                  style={{ transition: "color 0.2s", color: "white" }}
                >
                  {chat.name}
                </h3>
                <span className="font-tech text-[9px] text-gray-600 shrink-0 ml-2 tracking-wider">
                  {chat.time}
                </span>
              </div>
