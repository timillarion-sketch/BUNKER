...
import { api, encryptMessage, decryptMessage, getOrCreateKeyPair } from "@/core";
...
async function sendToBackend(characterId: string, message: string, encrypted: boolean): Promise<string> {
  const data = await api.post<{ reply: string }>("/ai/chat", { message, characterId, encrypted });
  return data.reply;
}
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    es.addEventListener("typing", (e) => {
      try {
        ...
        if (data.characterId === char.webhookName || data.characterId === char.id) {
          setAiTyping(data.typing === true);
        }
  ...
  // Load messages from backend and decrypt
  useEffect(() => {
    const charId = CHARACTER_ID_MAP[char.id] ?? char.id;
    api.get<{ messages: Msg[]; total: number }>(`/messages/${charId}`)
      .then(async data => {
        if (data.messages.length > 0) {
          ...
          setMessages(prev => [prev[0], ...decrypted]);
        }
      })
      .catch(() => {});
  }, [char.id]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Encrypt message with own public key (mandatory — plaintext never reaches the server)
      const keys = await getOrCreateKeyPair();
      const ciphertext = await encryptMessage(text, keys.publicKey);

      // Store encrypted message in DB
      await api.post(`/messages/${charId}`, { content: ciphertext, encrypted: true });

      // Send to AI (backend receives encrypted content)
      const reply = await sendToBackend(charId, ciphertext, true);
      setMessages(prev => [...prev, {
        id:        `a_${Date.now()}`,
        role:      "assistant",
        content:   reply,
  ...
  return (
    <div className="flex flex-col h-screen relative overflow-hidden" style={{ background: "#050508" }}>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-4 z-10 no-scrollbar">
        ...
        {/* Typing status from SSE */}
        <AnimatePresence>
          {aiTyping && (
            <motion.div key="typing-sse"
        ...
        {/* Local loading dots (fallback) */}
        <AnimatePresence>
          {loading && !aiTyping && (
            <motion.div key="typing-local"
              initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              className="flex justify-start">
              <div className="flex-shrink-0 mr-2 mt-1">
                <div className="w-7 h-7 rounded-sm flex items-center justify-center text-sm"
