...
import { Lock, Shield, Plus, Radio, Wifi, Hexagon, X, UserPlus, WifiOff } from "lucide-react";
...
import { useMeshSocket } from "@/hooks/use-mesh-socket";
...
function MeshBar({ meshEnabled, onToggle }: { meshEnabled: boolean; onToggle: (enable: boolean) => void }) {
  const [scanning, setScanning] = useState(false);
  const [expanded, setExpanded] = useState(false);
  const { status: socketStatus } = useMeshSocket(meshEnabled);

  const active = meshEnabled;
  ...
  const statusLabel = active   ? "Mesh-сеть активна (Ожидание пиров)"
                     : scanning ? "Сканирование..."
                     :            "Узлов в радиусе: 0";
  const socketLabel = socketStatus === "connected"   ? "Сигнальный сервер: Подключен"
                    : socketStatus === "connecting"  ? "Сигнальный сервер: Подключение..."
                    : socketStatus === "error"       ? "Сигнальный сервер: Ошибка"
                    :                                  "Сигнальный сервер: Отключен";
  const socketColor  = socketStatus === "connected"   ? "#00ff88"
                     : socketStatus === "connecting"  ? "#ffd700"
                     : socketStatus === "error"       ? "#ff3366"
                     :                                  "#666";
  ...
  return (
    <motion.div layout className="mb-5 rounded-sm overflow-hidden"
          ...
          {meshEnabled && (
            <div className="flex items-center gap-1.5">
              {socketStatus === "connected" ? (
                <Wifi className="w-3 h-3" style={{ color: socketColor }} />
              ) : socketStatus === "error" ? (
                <WifiOff className="w-3 h-3" style={{ color: socketColor }} />
              ) : (
                <Radio className="w-3 h-3" style={{ color: socketColor }} />
              )}
              <span className="font-tech text-[9px] tracking-wider" style={{ color: socketColor }}>
                {socketLabel}
              </span>
            </div>
          )}
      ...
      <div className="px-4 pb-4">
        <motion.button onClick={handleToggle} disabled={scanning} whileTap={{ scale: 0.97 }}
...
export default function ChatsList() {
  ...
  const [meshEnabled, setMeshEnabled] = useState(user?.meshEnabled ?? false);

  const handleMeshToggle = useCallback(async (enable: boolean) => {
    setMeshEnabled(enable);
    try {
      await api.put("/auth/settings", { meshEnabled: enable });
    } catch {
      setMeshEnabled(!enable);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen pb-28 px-4 pt-12 no-scrollbar overflow-y-auto">
      ...
      <MeshBar meshEnabled={meshEnabled} onToggle={handleMeshToggle} />
