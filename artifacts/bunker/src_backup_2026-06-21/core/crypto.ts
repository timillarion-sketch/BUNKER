...
export class CryptoService {
  ...
  async generateKeyPair(): Promise<{ publicKey: string; privateKey: string }> {
    ...
    this.storage.set(KEY_STORAGE_KEY, privateKeyBase64);
    this.storage.set(PUBLIC_KEY_STORAGE_KEY, publicKeyBase64);

    return { publicKey: publicKeyBase64, privateKey: privateKeyBase64 };
  }

  async getOrCreateKeyPair(): Promise<{ publicKey: string; privateKey: string }> {
    const storedPrivateKey = this.storage.get(KEY_STORAGE_KEY);
    const storedPublicKey = this.storage.get(PUBLIC_KEY_STORAGE_KEY);
  ...
  async decryptMessage(ciphertextBase64: string): Promise<string> {
    const privateKeyBase64 = this.storage.get(KEY_STORAGE_KEY);
    if (!privateKeyBase64) {
